...
<h2 style="text-align:center;color:red">Sketch最火爆的插件————Measure</h2>

<p style="text-indent:2em">对于前端仔，疲于页面业务逻辑的实现，当时是希望能在网页ui实现上，越快速实现越好。icon的尺寸大小、用色、字体大小、间距等等这些细节上的确认，为了减小前端仔和ui大大的沟通成本，如果能在图上一次性加入一些标注信息和切图等，这将大大提高前端er的工作效率 </p>
<p style="text-indent:2em">历来都是传输psd文件，前端要自己去量多每个图层的大小，为此，隆重推荐用Sketch插件————Measure.！！！从此妈妈再也不用担心我罗里吧嗦的繁琐切图问题了~</p>

<div>补充Measure的下载方法。。。。。。（待续）</div>
